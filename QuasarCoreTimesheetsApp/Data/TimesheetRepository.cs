...
namespace QuasarCoreTimesheetsApp.Data
{
    public class TimesheetRepository: ITimesheetRepository
    {
        ...
        public void CreateTimesheet(Timesheet timesheet)
        {
            ...
            {
                var id = conn.ExecuteScalar<int>(@"INSERT INTO " +
                            "timesheet(userid,date,starttime,endtime,comment,absence " +
                            "VALUE(@UserId, @Date, @StartTime, @EndTime, @Comment, @Absence); " +
                            "SELECT LAST_INSERT_ID();", timesheet);
                timesheet.Id = id;
            }
        }
        ...
        public Timesheet GetTimesheet(int userId, DateTime date)
        {
            using (var conn = OpenConnection())
            {
                return conn.QuerySingleOrDefault<Timesheet>("SELECT * FROM timesheet WHERE id=@id AND date= @date", 
                       new { 
                           userId,
                           date.Date
                       });
        ...
        public IEnumerable<Timesheet> GetTimesheets(int userId, DateTime startDate, DateTime endDate)
        {
            using (var conn = OpenConnection())
            {
                return conn.Query<Timesheet>("SELECT * FROM timesheet WHERE userId=@userId " +
        ...
        public IEnumerable<UserTimesheet> GetTimesheets(DateTime startDate, DateTime endDate)
        {
            using (var conn = OpenConnection())
            {
                return conn.Query<UserTimesheet>("SELECT * FROM timesheet.*, user.displayname FROM timesheet" +
                                            "JOIN aspnetusers user ON user.id = timesheet.userid" +    
                                            "WHERE timesheet.date >= @startDate AND timesheet.date <= @endDate",
                                               new
                                               {
                                                   startDate = startDate.Date,
                                                   endDate = endDate.Date
